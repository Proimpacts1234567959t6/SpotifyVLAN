...
module.exports = async function handler(req, res) {
  ...
  // Success redirect: use SPOTIFY_WEB_BASE, or derive from the callback request URL (where Spotify sent the user)
  let base = (process.env.SPOTIFY_WEB_BASE || '').trim().replace(/\/$/, '');
  if (!base && req.headers) {
    const origin = (req.headers.origin || req.headers.Origin || '').trim();
    if (origin) base = origin.replace(/\/$/, '');
    else if (req.headers.host) {
      const host = (req.headers.host || '').trim();
      const proto = (req.headers['x-forwarded-proto'] || req.headers['x-forwarded-protocol'] || 'https').toString().split(',')[0].trim();
